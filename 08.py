...
def get_num_map(line):
    ...
    for num in line:
        if len(num) == 2:
            ...
            num_map["8"] = num
    for num in line:
        if num in num_map.values():
            continue
        if len(num) == 5 and set(num_map["1"]) < set(num):
            num_map["3"] = num
            break
    for num in line:
        if num in num_map.values():
            continue
        if len(num) == 6 and set(num_map["3"]) < set(num):
            num_map["9"] = num
        elif len(num) == 6 and set(num_map["1"]) < set(num):
            num_map["0"] = num
    for num in line:
        if num in num_map.values():
            continue
        if len(num) == 6 and num not in num_map.values():
            num_map["6"] = num
            break
    for num in line:
        if num in num_map.values():
            continue
        if len(num) == 5 and set(num) < set(num_map["6"]):
            num_map["5"] = num
            break
    for num in line:
        if num in num_map.values():
            continue
        num_map["2"] = num
    num_map = dict(("".join(sorted(v)), k) for k, v in num_map.items())
    return num_map
...
def part2(lines):
    sum = 0
    for line in lines:
        num_map = get_num_map(line[0])
        output = [num_map["".join(sorted(num))] for num in line[1]]
        sum += int("".join(output))
    return sum
